...
int main() {
  ...
  assert(rx.Claim("joe"));
  assert(!rx.Claim("someone-else"));      // must not evict, must not queue
  assert(rx.HeldBy("joe"));
  assert(rx.Holder() == "joe");
  ...
  assert(rx.Accept(bytes, frame.size() * 2, /*keyed=*/true));
  ...
  // not a subtle glitch.
  assert(!rx.Accept(bytes, 961, /*keyed=*/true));
  assert(!rx.Accept(bytes, 0, /*keyed=*/true));
  assert(rx.Accepted() == 1);             // neither was counted
  ...
  assert(rx.Dropped() == 0);
  assert(rx.QueueDepth() > TxAudioReceiver::kMaxQueuedChunks);
  ...
  assert(pumped > 0);
  assert(raw->FramesWritten() > before);
